...
这是开源项目wordle+的fork，非常感谢原作者的开源项目。

词库替换为与编程/算法有关的五字词。

运行方法：安装npm后，使用`npm i`安装依赖。此后每次修改，`npm run build`即可。运行`npm run start`可以将其部署在网站上。

运行的默认端口为8080，如果需要更换端口（例如更换为3000），使用`npm run start -- -p 3000`即可。
